...
**удаление фона** и **апскейл**. Обработка идёт целиком на вашем компьютере —
изображения никуда не отправляются. См. [Что уходит в сеть](#что-уходит-в-сеть).
...
Каждая загрузка привязана к **контрольной сумме SHA-256**, которая проверяется до того,
как файл будет использован. При несовпадении загрузка удаляется и показывается ошибка —
подменённый или недокачанный артефакт не распакуется и не запустится.

---

## Что уходит в сеть

Ваши изображения не покидают компьютер: никакой загрузки на сервер, телеметрии,
аналитики, отчётов о падениях и аккаунтов. Приложение открывает ровно три вида
соединений, все — к публичным, заранее прописанным адресам:
...
Если выключить проверку обновлений, приложение становится полностью молчаливым: при
уже скачанных моделях оно не откроет ни одного соединения по своей инициативе.
